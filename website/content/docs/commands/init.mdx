...
8. **TypeScript type safety** — generate `next-intl.d.ts` for key autocomplete (keys mode only)
...
In keys mode, the wizard also:

- Installs **next-intl** automatically
- Creates **`i18n/request.ts`** with locale detection setup
- Updates **`next.config.ts`** with `createNextIntlPlugin`

### Inline mode

In inline mode, the wizard also:
