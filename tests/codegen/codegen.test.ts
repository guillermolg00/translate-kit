...
describe("codegen integration", () => {
	...
	it("skips file when transform produces invalid syntax", async () => {
		...
		const _originalTransform = transformModule.transform;
...
export function Footer() {
  ...
  return <ul>{formatted.map(l => <li key={l.href}>{l.title}</li>)}</ul>;
	...
	it("module factory: .mts files are parsed as TypeScript", async () => {
		const filePath = join(tempDir, "data.mts");
		await writeFile(
			filePath,
  ...
  { title: "About", href: "/about" },
	...
	it("module factory: .js files do NOT get TypeScript type annotations", async () => {
  ...
  { title: "About", href: "/about" },
		...
		const content = await readFile(filePath, "utf-8");
		expect(content).not.toContain("t: any");
		expect(content).toContain("t =>");
	});
});
