...
import { createUsageTracker, formatUsage, formatCost, estimateCost } from "../src/usage.js";
...
vi.mock("tokenlens", () => ({
  ...
  getTokenCosts: vi.fn(
    (args: { modelId: string; usage: { prompt_tokens: number; completion_tokens: number }; providers: any }) => ({
      inputUSD: args.usage.prompt_tokens * 0.0000025,
      outputUSD: args.usage.completion_tokens * 0.00001,
      totalUSD: args.usage.prompt_tokens * 0.0000025 + args.usage.completion_tokens * 0.00001,
    }),
  ),
}));
...
describe("createUsageTracker", () => {
  it("starts at zero", () => {
    ...
    expect(tracker.get()).toEqual({ inputTokens: 0, outputTokens: 0, totalTokens: 0 });
  });
  ...
  it("accumulates multiple add() calls", () => {
    ...
    expect(tracker.get()).toEqual({ inputTokens: 300, outputTokens: 125, totalTokens: 425 });
  });
  ...
  it("handles undefined fields gracefully", () => {
    ...
    expect(tracker.get()).toEqual({ inputTokens: 10, outputTokens: 20, totalTokens: 30 });
  });
});

describe("formatUsage", () => {
  it("formats usage correctly", () => {
    const result = formatUsage({ inputTokens: 1500, outputTokens: 300, totalTokens: 1800 });
