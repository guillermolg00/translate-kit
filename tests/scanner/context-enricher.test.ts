...
import { deriveRoutePath, enrichStrings } from "../../src/scanner/context-enricher.js";
...
describe("deriveRoutePath", () => {
  it("extracts route from Next.js app router", () => {
    ...
    expect(deriveRoutePath("src/app/settings/profile/page.tsx")).toBe("settings.profile");
  });
...
describe("enrichStrings", () => {
  const makeStr = (overrides: Partial<ExtractedString> = {}): ExtractedString => ({
    text: "Hello",
    type: "jsx-text",
  ...
  it("detects sectionHeading from h1-h3 tags", () => {
    const strings = [
      ...
      makeStr({ text: "Click here", parentTag: "button", componentName: "Hero" }),
    ];
