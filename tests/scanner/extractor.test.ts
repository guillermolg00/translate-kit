...
describe("extractor", () => {
	describe("simple-component.tsx", () => {
		...
		it("identifies parent tags", () => {
			...
			const h1String = strings.find(
				(s) => s.text === "Welcome to our platform",
			);
	...
	describe("template literals", () => {
		it("extracts template literal in JSX expression", () => {
			const code =
				"function App({ name }) { return <p>{`Hello ${name}`}</p>; }";
			const ast = parseFile(code, "test.tsx");
			const strings = extractStrings(ast, "test.tsx");
		...
		it("extracts template literal in object property", () => {
			const code =
				"function App({ id }) { const item = { title: `Task ${id}` }; return <div />; }";
			const ast = parseFile(code, "test.tsx");
			const strings = extractStrings(ast, "test.tsx");
		...
		it("extracts template literal with MemberExpression", () => {
			const code =
				"function App({ user }) { return <p>{`Welcome ${user.name}`}</p>; }";
			const ast = parseFile(code, "test.tsx");
			const strings = extractStrings(ast, "test.tsx");
	...
	describe("conditional expressions", () => {
		...
		it("extracts ternary with TemplateLiteral branch", () => {
			const code =
				'function App({ isAdmin, name }) { return <p>{isAdmin ? `Hello ${name}` : "Guest"}</p>; }';
			const ast = parseFile(code, "test.tsx");
			const strings = extractStrings(ast, "test.tsx");
	...
	describe("module-level object properties", () => {
      ...
      `;
			...
			const moduleProps = strings.filter(
				(s) => s.type === "module-object-property",
			);
